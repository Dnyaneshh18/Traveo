...
export function RatingModal({ data, visible, onClose }: Props) {
  ...
  const submit = useMutation({
    ...
    onSuccess: () => {
      toast('Feedback submitted! ⭐', undefined, 'success');
  ...
  const title = 'How were the passengers?';
  const subtitle = `${data.passenger_count || 1} student(s) · ${data.destination_address || 'Completed Trip'}`;
  const ratingTags = ['Great students', 'Cooperative', 'On time for pickup', 'Polite & respectful'];
      ...
      <View style={styles.backdrop}>
        <View style={styles.card}>
          ...
          <Caption center style={{ marginBottom: 4 }}>
            {stars === 5 ? 'Excellent group!' : stars === 4 ? 'Good passengers' : stars === 3 ? 'Average' : 'Could be better'}
          </Caption>
          ...
          <TextInput
            placeholder="Feedback for passengers (optional)…"
          ...
          <Button
            title={submit.isPending ? 'Submitting…' : 'Submit rating'}
            onPress={() => submit.mutate()}
            loading={submit.isPending}
            style={{ width: '100%', marginTop: spacing.sm, backgroundColor: colors.driver }}
          />
